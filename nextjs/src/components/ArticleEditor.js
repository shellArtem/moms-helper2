...
// --- ДИНАМИЧЕСКИЙ ИМПОРТ CROPPER'а ---
// Это гарантирует, что компонент будет загружен только на клиенте.
...
const ArticleEditor = ({ articleId }) => {
    ...
    useEffect(() => {
        if (isEditing) {
            ...
            setIsLoading(false); // Для новой статьи загрузка не нужна
        }
    }, [articleId, isEditing]);
    ...
    // Показываем загрузчик, пока данные статьи не загружены
    if (isLoading) {
        return <div className="admin-container"><p>Загрузка редактора...</p></div>;
    }
    ...
    return (
        <div className="admin-container">
            ...
            <form onSubmit={handleSubmit} className="admin-form">
                ...
                {/* --- Обертка, чтобы ReactCrop рендерился только когда нужно --- */}
                {isCropping && imgSrc && (
                    <div className="cropper-container">
                        <h3>Обрежьте изображение (16:9)</h3>
                        <ReactCrop
